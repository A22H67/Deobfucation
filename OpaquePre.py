    ...
    def code_walk(addr,symbols,conds,depth,final_states,path):
        ...
        for _ in range(uncond_limit):
            ...
            if isinstance(pc,ExprCond):
                # convert condition to symbol
                cond_true={pc.cond: ExprInt(1,32)}
                cond_false={pc.cond: ExprInt(0,32)}
                #Run the Miasm's simplification engine on the address to be sure to have the simplest expression
...
def OpDeob():
    ...
    #disassemble block funcation at target address
